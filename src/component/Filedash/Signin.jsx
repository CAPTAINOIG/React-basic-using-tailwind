...
const Signin = () => {
  ...
  return (
    <section id='background' className='flex flex-col pb-20 items-center justify-center'>
      <div className='content bg-white shadow rounded w-[30%] p-5 mt-10'>
        <div className=''>
          <img className='mx-auto' src={logodark} alt="" />
        </div>
        <h1 className='my-3 text-center text-black'>Sign In</h1>
        <form onSubmit={formik.handleSubmit}>
          <div className='my-7 h-[50px]'>
            <input className='border mt-1 rounded border-blue-700 py-2 px-4 w-full focus:outline-blue-500' onBlur={formik.handleBlur} onChange={formik.handleChange} type="email" placeholder="email" name='email' />
            <span className='text-red-500 text-sm'>{formik.touched.email && formik.errors.email}</span>
          </div>
          <div className='relative my-7 h-[50px]'>
            <input className='border rounded border-blue-700 py-2 px-4 w-full focus:outline-blue-500' type={isPasswordVisible ? 'text' : 'password'} placeholder="Password" onBlur={formik.handleBlur} onChange={formik.handleChange} name='password' />
          ...
          <button type='submit' className='btn btn-primary w-full bg-blue-700 p-2 rounded text-white'>Sign in</button>
        </form>
        <div className='border-b border-gray-300 my-5'></div>
        <p class="text-muted text-center">Login with your social media account.</p>
        <ul className='flex text-blue-700 gap-7 my-5 px-5'>
