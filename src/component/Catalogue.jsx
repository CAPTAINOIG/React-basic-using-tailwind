...
const Catalogue = () => {
  ...
  const handleSubmit = (e) => {
    ...
    filterCatalogueProduct();
    setFormFilters({ ...formFilters, title: "", selectedPriceRange: "all" });
  };
  ...
  const filterCatalogueProduct = () => {
    ...
    if (filterType && filterValue !== "") {
      if (filterType === "Greater than") {
        ...
        filteredItems = filteredItems.filter((item) =>
          item.name.toLowerCase().endsWith(filterValue.toLowerCase())
        );
      } else if (filterByUPC) {
          ...
          >
            {Object.keys(priceRanges).map((range, index) => (
              <option key={index} value={range}>
                {range === "all"
                  ? "All"
                  : `$${priceRanges[range].min} - $${priceRanges[range].max}`}
              </option>
            ))}
          </select>
